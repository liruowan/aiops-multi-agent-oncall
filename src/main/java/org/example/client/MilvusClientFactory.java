...
public class MilvusClientFactory {
    ...
    public MilvusServiceClient createClient() {
        ...
        try {
            ...
            String memoryCollectionName = memoryProperties.getCollectionName();
            if (!collectionExists(client, memoryCollectionName)) {
                logger.info("collection '{}' 不存在，正在创建...", memoryCollectionName);
                createMemoryCollection(client);
                createMemoryIndexes(client);
                logger.info("成功创建 memory collection '{}'", memoryCollectionName);
            }
            loadCollection(client, memoryCollectionName);
    ...
    private void createMemoryCollection(MilvusServiceClient client) {
        ...
        R<RpcStatus> response = client.createCollection(CreateCollectionParam.newBuilder()
                .withCollectionName(memoryProperties.getCollectionName())
                .withDescription("OnCall long-term memory collection")
                .withSchema(schema)
                .withShardsNum(MilvusConstants.DEFAULT_SHARD_NUMBER)
                .build());
        if (response.getStatus() != 0) {
    ...
    private void createMemoryIndexes(MilvusServiceClient client) {
        R<RpcStatus> response = client.createIndex(CreateIndexParam.newBuilder()
                ...
                .build());
        if (response.getStatus() != 0) {
            throw new RuntimeException("创建 memory vector 索引失败: " + response.getMessage());
        }
    }
